...
/**
 * 
 * A simple client to connect to a SOR. It sends the given request string to the
 * url defined
 * 
	...
	private static XmlObject doSend(String requestUrl, String request,
		...
		if (requestMethod.equals(GET_METHOD)) {
			...
			postMethod.setRequestEntity(new StringRequestEntity(request,
					SupervisorProperties.getInstance()
							.getClientRequestContentType(),
					SupervisorProperties.getInstance()
							.getClientRequestEncoding()));
			method = postMethod;
		} else {
